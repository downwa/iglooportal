...
<html>
    ...
    <script type="text/javascript"><!-- 
			...
			function grantAccess() { // Retry the page
				try {
					if(!accessGranted) {
						...
						grantaccess.src="https://reserve.bristolinn.com/iglooportal/status.php?session=<?=$session?>&redirect=<?=$redirect?>";
			...
			function statusKeepAlive() {
				try {
					var grantaccess=document.getElementById('grantaccess');
					grantaccess.src="https://reserve.bristolinn.com/iglooportal/status.php?session=<?=$session?>&count=9999&redirect=<?=$redirect?>";
			...
			function frameReady(frameObject) { // Grant access page loaded...
				try {
					//document.location="https://reserve.bristolinn.com:8444/status/index.php?session=<?=$session?>&count=0&redirect=<?=$redirect?>";
	...
	<body onload="setTimeout('grantAccess()',5000);">
		<iframe id="grantaccess" src="https://reserve.bristolinn.com/iglooportal/status.php?session=<?=$session?>&redirect=<?=$redirect?>" onload="frameReady(this);" width="640" height="320" scrolling="no" frameBorder="0"><!-- style="display:none" -->
			<a href="https://reserve.bristolinn.com/iglooportal/status.php?session=<?=$session?>&redirect=<?=$redirect?>">Click to access session</a>
		</iframe>
	</body>
</html>
